This folder contains metrics for the Origin-Destination to and from the zones of the named analysis, and a selected geography such as a Traffic Analysis Zone (TAZ), Census Block Group, or ZIP Code in US or Dissemination Areas in Canada.
...
odg_*.csv
=========
This files contains the metrics.
...
- Origin Zone Source: This refers to whether the origin zone was provided as a geography or inputted by the user.
- Origin Zone is Bi-Direction: Indicates if the zones are bi-directional. Values are "Yes" or "No".
...
- Destination Zone Source: This refers to whether the destination zone was provided as a geography or inputted by the user.
- Destination Zone is Bi-Direction: Indicates if the zones are bi-directional. Values are "Yes" or "No".
...
- O-D Traffic: The volume of trips from the geography to the zone (if the value in column Geography Location is origin), or the volume of trips from the zone to the geography (if the value in column Geography Location is Destination), measured using one of the output units described above under the Output Unit Terms.
...
- Avg Travel Time (sec): The average trip time in seconds for all trips starting in, passing through, or ending in the zone based on the zone Intersection Type value, for low network factor* trips.
- Avg All Travel Time (sec): The average trip time in seconds for all trips starting in, passing through, or ending in the zone based on the zone Intersection Type value.
- Avg Trip Length (mi or km): The average trip length in miles for all trips starting in, passing through, or ending in the zone based on the zone Intersection Type value, for low network factor* trips.
- Avg All Trip Length (mi or km): The average trip length in miles for all trips starting in, passing through, or ending in the zone based on the zone Intersection Type value.
...
=====================
Standard geographies refer to the following:
- TAZ: 2010 Census Traffic Analysis Zones
- Census Block Group: 2010 Census Block Groups
- Zip Code: 2010 Census Zip Code Tabulation Areas
- Dissemination Areas in Canada

Geographies can be downloaded from the US Census as the 2010 version of the TIGER/Line Shapefiles. The join is done using the GEOID field.

OD Pairs with No Values
=======================
If the output unit values for an OD pair for a specific time period (e.g. Average Weekday, Early AM) are below StreetLight's significance threshold, no results will be shown in the odg_(taz|zip|blkgrp)_*.csv file.
